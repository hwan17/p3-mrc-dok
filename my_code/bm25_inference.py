...
def run_bm25_retrieval(datasets, training_args, topk):
    ...
    return datasets, doc_scores


def run_mrc(data_args, training_args, model_args, datasets, tokenizer, model, doc_scores):
    ...
    if training_args.do_predict:
        predictions = trainer.predict(test_dataset=eval_dataset,
                                        test_examples=datasets['validation'], doc_scores=doc_scores)
